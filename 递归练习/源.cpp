...
{
	cin>>G.n>>G.m;
	G.edge_time=new int* [G.m];
	for(int i=0;i<G.m;i++)
		G.edge_time[i]=new int [G.m];
	G.edge_bitcoin=new int* [G.m];
	for(int i=0;i<G.m;i++)
		G.edge_bitcoin[i]=new int [G.m];
	for(int i=0;i<G.n;i++)
		for(int j=0;j<G.n;j++)
		{
			if(i==j)
...
int main()
{
	...
	int** cost_time;
	int** cost_bitcoin;
	cost_bitcoin=new int* [G.m];
	for(int i=0;i<G.m;i++)
		cost_bitcoin[i]=new int [G.m];
	cost_time=new int* [G.m];
	for(int i=0;i<G.m;i++)
		cost_time[i]=new int [G.m];
	int i,j,k;
	for(i=0;i<n;i++)
		for(j=0;j<n;j++)
		{
